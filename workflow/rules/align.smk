rule star_index:
    ...
    """
    input:
        fasta = ref_path("genome", "fasta"),
        gtf   = ref_path("genome", "gtf")
    output:
        indexdir = directory(
            resolve_results_filepath("star", config["star"]["index_dir"])
        ),
        length = resolve_results_filepath("star", f"{config['star']['index_dir']}/chrLength.txt")

    params:
        indexdir = resolve_results_filepath("star", config["star"]["index_dir"]),
        sjdb_overhang = config["star"]["sjdb_overhang"]
    log:
        resolve_logs_filepath("star", "star_index.log")
    threads:
...
rule star_align_pe:
    ...
    output:
        bam = resolve_results_filepath("star", "{sample}/{sample}.bam")
    params:
        outprefix = resolve_results_filepath("star", "{sample}/{sample}.pe."),
        extra     = config["star"].get("extra_params", "")
    log:
        resolve_logs_filepath("star", "{sample}.star.pe.log")
    threads:
        conservative_cpu_count()
    conda:
        resolve_envs_filepath("star.yaml")
...
rule star_align_se:
    ...
    output:
        bam = resolve_results_filepath("star", "{sample}/se/{sample}.bam")
    params:
        outprefix = resolve_results_filepath("star", "{sample}/se/{sample}.se."),
        extra     = config["star"].get("extra_params", "")
    log:
        resolve_logs_filepath("star", "{sample}.star.se.log")
    threads:
        conservative_cpu_count()
    conda:
        resolve_envs_filepath("star.yaml")
